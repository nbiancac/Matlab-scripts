...
%
% Pftoa.m:  Alternative number conversion formatting - version 3Dec13
...
%
if nargin ~= 2
  disp('Calling sequence: resultString = Pftoa(formatString,val)');
  if nargin==1 % Call with just one argument to run Pftoa test code
    ...
    s = 'Pftoa test complete. Test file written';  fclose(fi); % done with test code
  end
  return;
end;
% if length(val) ~= 1 disp('2nd argument of Pftoa must be a scalar'); return; end;
