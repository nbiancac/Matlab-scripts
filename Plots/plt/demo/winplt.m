...
case 0,  % no arguments: Create the winplt window
  ...
  S.tr = plt(0,zeros(1,6),'FigName','winplt (FFT window shapes)',...
    'xy',[0 .143 .093 .787 .798; -1 .006 .723 .077 .217],...
    'Styles','-:-:-:-:','TIDcback',@TIDbk,'Ylim',ylim,'YlimR',3.51+ylim/40,...
    'LabelX','Bin number','Options','-Y','Right',[2 4 6],'User','--',...
    'LabelY',{'Frequency shape (dB)' 'Time shape'},'AxisLink',0,...
    'Pos',[10 45 750 550],'DisTrace',[0 0 1 1 1 1],'TRACEc',TRACEc);
  ...
  text(.97,-.08,VerString,'units','norm','color',[.4 .4 .4],'fontsize',8);
  ...
  S.sel = plt('pop',[.08 .02 .38 .96],winplt(-2),'offset',30,'index',2,'callbk',@winCB,...
         'fontsize',18,'hide',[findobj(gcf,'type','uicontrol'); findobj(gcf,'type','line')]);
  S.cor = uicontrol('Style','CheckBox','Units','Norm','Pos',[.969 .58 .03 .04],...
  ...
  S.ker = plt('pop',[.005 .695 .16 .3],c,'offset',[-.02 8.5],'callbk',@kerCB,...
           'enable',0,'color',[1 1 1],'fontsize',9,'interp','tex','hide',0,'swap');
...
function helpWinP(in1,in2)
  if ispc
    f = which('plt.chm');
    if length(f)  % on a PC, open plt.chm pointing at the winplt topic
      dos(['hh mk:@MSITStore:' f '::/pltfiles/winplt/winplt.htm &'])
      return;
    end;
  end;
  f = which('plt.m'); % come here if not a PC or the chm file wasn't found
  if isempty(f) return; end;  % should not happen!
  web(['file:///' f(1:end-5) 'pltfiles/winplt/winplt.htm'],'-browser');
