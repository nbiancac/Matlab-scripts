...
function dice(in1)
  ...
  p = [.127 .931 .071 .042;  % position: 1 bet
       .241 .931 .100 .042;  % position: Continuous
       .384 .931 .057 .042;  % position: Clear
       .191 .874 .250 .034]; % position: speed slider
...
function btn(h,arg2,arg4) % button callbacks -------------------------------------------
  ...
  while ishandle(S.betC)& strcmp(get(S.betC,'string'),'Stop')
    ...
    pause(1.001-get(S.sli,'value'));    % adjust the speed of the loop
  end;
% end function btn
