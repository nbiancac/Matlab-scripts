...
%   running by entering a new number in the "# of lines" edit box (bottom center).
...
function pltn(Nlines)
  ...
  if Nlines>99 TraceID = [];
        ...
        if     Nlines>48  n3 = floor(Nlines/3); n3 = [n3 n3];
        elseif Nlines>24  n3 = floor(Nlines/2);
        end;
  end;
  ylim = [min(y(end,:)) max(y(1,:))] + [-.1 .1];
  S.tr = plt(t,y,'Xlim',t([1 end]),'TraceID',TraceID,'TIDcolumn',n3,...
      'LabelX',[' hours past ' timeRef],'Pos',[10 50 870 700],...
  ...
  plt('cursor',S.cid,'update');  % update xstring
  p = [505 6 45 20;  % position: Edit box
       435 8 65 15]; % position: Edit box label
  set([uicontrol uicontrol],{'style'},{'edit';'text'},{'pos'},num2cell(p,2),...
    'backgr',[3 3 3]/4,'foregr','blue',{'str'},{prin('%d',Nlines); '# of lines:'},...
    'fontw','bold','callback','a=str2num(get(gcbo,''str'')); close(gcf); pltn(a);');
% end function pltn
...
function xstrCB(h,arg2,S)  % xstring edit box callback
  ...
  plt('cursor',S.cid,'update',k);            % move cursor to the desired spot
%end function xstrCB