...
% * Shows the advantage of setting the line data after the call to plt.
...
%   slider (which appears just below the peak/valley finder buttons).
...
if ~nargin                      % Initialize gauss window section ------------
  ...
  if isempty(f) % create the zoom window if it doesn't already exist
     ...
     plt(xy{:},'TraceID',traceID,'LabelX','','LabelY','','Options','-All',...
         'DIStrace',dis,'FigName',zoomFig,'Pos',[720 45 425 525],...
