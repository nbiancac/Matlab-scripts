...
function movbar(in1);
  ...
  S.rnd = uicontrol('pos',[10 420 40 25],'string','Rand','Callback',{@btn,1}); % Rand button
  S.wlk = uicontrol('pos',[10 380 40 25],'string','Walk','Callback',{@btn,0}); % Walk button
  S.tx  = text(-.5,980,...
        '\leftarrow click/drag on this green line to move the horizontal threshold',...
        'color',[0 1 .6]);
  set(gcf,'user',S);                                   % save for callbacks
  set(findobj('tag','xstr'),'fontsize',16,'str',xstr); % initialize xstring
...
function t = xstr()  % xstring function: move threshold bar
  ...
  xy = plt('cursor',S.cid,'get','pos');  y = imag(xy);
  set(S.hz,'y',[y y]);
...
function btn(h,arg2,r)  % rand and walk button callbacks
  ...
  if r walk(S.tr(2),0);
       ...
       set(S.wlk,'string',t);  % toggle Walk/Stop 
...
function walk(h,r) % set the y data for line h (rand button use 2 argument form)
  ...
  plt('cursor',getappdata(gcf,'cid'),'update');    % update xstring
  drawnow;
% end function walk
