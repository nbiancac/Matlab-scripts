...
% * The 'xy' argument is used to widen the TraceID box to make room for
%   the longer than usual trace ID names, as well as to move the MenuBox to
%   make room for the character data table
...
q = [10 50 100+sz(3:4)/2];    % figure position (depends on screen size)
p = [-1 .004 .710 .083 .278;  % position: TraceID box
     -2 .012 .444 .062 .254]; % position: Menubox
plt('TraceID',Id,'Pos',q,'xy',p,'FigName','pltvbar',...
    'FigBKc',[.1 .1 .2],'xlim',[-.1 6.2],...
...
fs = round(q(3)/75 - 2);                 % best font size for this figure size
