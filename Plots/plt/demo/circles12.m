...
plt(cN*r + repmat(c,N,1),'FigName','12 Circles (solution 3)',...
    'LineWidth',2,'Options','Ticks','Pos',[10 sz(4)-745 640 669],...
...
for k = [2 1]
  ...
  plt(cN*r + repmat(c,N,1),'LabelY','','LabelX','',...
     ...
     'TraceID',ID,'LineWidth',2,'Pos',[10 50 600 0]);
end;
...
h = plt(e,cN,'Pos',[662 sz(4)-477 0 400],'Figname','12 circles (part 2)',...
        'Options','Ticks Nocur -All','TraceID',['div ';'circ'],...
        'LabelX','','LabelY','','Xlim',[-1.1 1.1],'Ylim',[-1.1 1.1]);
set(gca,'xtick',[],'ytick',[]);                 % remove tick marks
he = plt('edit',[150 8 35 16],[1 -15 15],'label','Rotation speed:');% rotation speed
bt = uicontrol('str','Stop','pos',[240 7 35 20],'callback','delete(gcbo)'); % stop btn
while ishandle(bt)             % forever loop (until you click on the stop button)
  s = plt('edit',he,'get');
  if s  if s<0 s=s+N-1; end;   % rotate CCW/CW is slider value is +/-
        e = e .* cN(round(s));
