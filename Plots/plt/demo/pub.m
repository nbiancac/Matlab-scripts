% pub.m ------------------------------------------------------------
%
% All the other plt examples in the demo folder use plotting formats
% appropriate for data exploration (the main design goal of plt). However
% plt can also use formats appropriate for creating plots for publication.
% This script demonstrates this by creating three different figure windows.
% Note that all 3 windows are created by calling "pltpub" which simply
% calls "plt" with several parameters optimized for creating publishable
% plots.
%
% The first window (plot 1 - upper) is a bar chart that demonstrates how to
% embed the plot data inside the script as comments. It also demonstrates
% the use of the prin function to display a table of random numbers in a
% text box. The vertical position of the plot depends on the screen size.
%
...
% bars, shows how to use the TraceID box as a legend. The special character
% ']' is used in the first TraceID to disable the shading of the trace name
% that is normally used to indicate the trace is on the right hand axis.
% Also the '+XtickLabel' parameter is used in the plt call to remove the
% tick labels on the x axis. Then an array of text objects are used to create
...
fclose(f);
set(0,'Units','pixels'); ss = get(0,'screensize'); py = min(570,ss(4)-500); % vertical position
pltpub(Pvbar(t,950,v),'Pos',[8 py 1140 440],'xy',[.069 .116 .641 .852],...
...
uicontrol('string','print','units','norm','pos',[.006 .015 .04 .05],...
...
uicontrol('style','text','units','norm','pos',[.72 .04 .273 .93],'background',[.9 1 .8],...
          'fontname','Lucida Console','string',s);    % display table of numbers in a text box
% plt move;                                           % switch to repositioning mode
...
p = [ 0 .062 .110 .846 .860;  % position: plotting axis
     -1 .360 .164 .248 .112]; % position: TraceID box
px = min(825,ss(3)-680);      % horizontal position
pltpub('Right',1:2,time,temp,Pebar(time,temp,tempL,tempU,60),...
    t3,e3,Pebar(t2,events,eventsL,eventsU,50),'LineWidth',2,...
   'xy',p,'+Ycolor','b','-Ycolor','r','TraceC',('rrbb')',...
   'FigName','Plot 3: Activity & Temperature vs Time of day',...
   'LabelY',{'# of Events' 'Temperature (\circC)'},'TIDc',[.8 1 .8],...
   'LabelX','Time of day (13-Feb-09)','+-<>.FontSize',14,...
   'Xlim',time([1 end])+[-.2 .2],'Ylim',{[0 100] [0 60]},...
   'TraceID',{']Temperature' '' 'Activity'},'TraceMK',.6,...
   '+XtickLabel',' ','Pos',[px 55 670 480],'Link',link);
