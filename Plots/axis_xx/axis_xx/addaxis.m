classdef addaxis < hgsetget & matlab.mixin.Heterogeneous
    ...
    methods(Abstract = true)
        obj = applyTransform(obj, handle, event);
    end
    ...
    methods
        ...
        function obj = plot(obj, varargin)
            ...
            obj.l_parent_lim = addlistener(obj.h_parent, ...
                lim, 'PostSet', @(h, ev) obj.applyTransform(h,ev));
        ...
        function obj = addaxis(varargin)
            ...
            hax = obj.h_parent;
            set(zoom(hfig),'ActionPostCallback',@(x,y)  addaxis.postZoomFcn(hax, y));
            set(zoom(hfig),'ActionPreCallback',@(x,y) addaxis.preZoomFcn(hax, y));
            set(pan( hfig),'ActionPostCallback',@(x,y)  addaxis.postZoomFcn(hax, y));
            set(pan( hfig),'ActionPreCallback',@(x,y) addaxis.preZoomFcn(hax, y));
            zoom reset;
            % Use custom datatips
            ...
            % Set the figure resize callback if needed...
            if isempty(get(hfig, 'ResizeFcn'))
                set(hfig, 'ResizeFcn', @(x,y) addaxis.resizeFigureCallback(x,y));
            end
    ...
    methods(Static = true)
        function preZoomFcn(handle, event)
            aad = addaxis.getAxes(handle);
            for i = 1:length(aad)
                aad(i).preZoom(handle, event);
            end
        end
        function postZoomFcn(handle, event)
            aad = addaxis.getAxes(handle);
            for i = 1:length(aad)
                aad(i).postZoom(handle, event);
            end
        end
        ...
        function resizeFigureCallback(hFig, event)
            % Figure resize callback function
            Children = get(hFig, 'Children');
            for i = 1:length(Children)
                if isfield(getappdata(Children(i)), 'addaxis_data')
                    addaxis.resizeParentAxis(Children(i));
                end
            end
        end
    end
