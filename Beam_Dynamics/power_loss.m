...
% [ploss,kloss]=power_loss(freq,Zlong,machine,tol,flagshow)
% for a Gaussian spectrum up to tol% of magnitude of the spectrum  
...
h=exp(-(omega.^2*taub^2));
    ...
    if isnan(Pmax)
        warning('Pmax=Nan, consider increasing the frequency range or decreasing the tolerance')
    end
p=1:Pmax;
